<template>
    <div class="min-h-screen bg-gray-100 py-6 flex flex-col justify-center sm:py-12">
      <div class="relative py-3 sm:max-w-xl sm:mx-auto">
        <h1 class="text-2xl font-semibold text-center mb-6">Edit Administrator</h1>
        <div class="relative px-4 py-10 bg-white mx-8 md:mx-0 shadow rounded-3xl sm:p-10">
          <form @submit.prevent="handleSubmit" class="space-y-4">
            ...
            <div>
              ...
              <span class="text-xs text-gray-500">Leave empty to keep the current password.</span>
            </div>
            <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline w-full">Update Administrator</button>
          </form>
        </div>
      </div>
    </div>
  ...
  export default {
    ...
    setup(props) {
      ...
      const handleSubmit = () => {
        form.put(route('admin.update', { id: props.administrator.id }));
      };
